...
import {
  PAYMENT_LIST_FORM
} from '@/utils/ADempiere/dictionary/field/search/payment.js'
...
/**
 * Use Payment as mixin
 * @param {*} recordRow
 * @returns
 */
export default ({
  uuidForm = PAYMENT_LIST_FORM,
  parentUuid,
  containerUuid,
  containerManager,
  fieldAttributes
  ...
  const showQueryFields = computed({
    set(newValue) {
      store.commit('setPaymentFieldShowQueryFields', {
        containerUuid: uuidForm,
        showQueryFields: newValue
      })
    },
    get() {
      return store.getters.getPaymentShowQueryFields({
        containerUuid: uuidForm
      })
    }
  })
