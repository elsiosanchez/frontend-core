...
<template>
  ...
  >
    <el-select
      ...
      @visible-change="loadBusinessPartnerGroups"
    >
      <empty-option-select
...
// API Request Methods
import { requestListBusinessPartnerGroups } from '@/api/ADempiere/fields/search/business-partner.ts'
...
export default defineComponent({
  name: 'BusinessPartnerGroupField',
  ...
  setup(props) {
    const ATTRIBUTE_KEY = 'business_partner_group_id'
    ...
    function loadBusinessPartnerGroups(isShowList) {
      if (!isShowList) {
        return
      }
      requestListBusinessPartnerGroups({
        pageSize: 100
      })
        .then(response => {
