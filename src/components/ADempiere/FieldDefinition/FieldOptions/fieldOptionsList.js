...
/**
 * See Addresses Business Partner
 */
export function SeeBusinessPartnerField({
  displayDefinition
}) {
  return {
    ...
    executeMethod: ({ containerManager, window, fieldAttributes, value, zoom }) => {
      ...
      store.commit('setShowBusinessPartner', true)
    },
    displayDefinition
  }
}
...
/**
 * Add Addresses Business Partner
 */
export function newBusinessPartnerField({
  displayDefinition
}) {
  const { is_insert_record, id } = displayDefinition
  return {
