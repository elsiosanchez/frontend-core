...
<template>
  ...
  >
    <template slot="label">
      {{ $t('form.outBoundOrder.process.freightDocumentTypes') }}
    </template>
...
// API Request Methods
import {
  requestListFreightDocumentTypes
...
export default defineComponent({
  name: 'FreightDocumentTypesField',
  ...
  setup() {
    const value = computed({
      // getter
      get() {
        const { freightDocumentTypesId } = store.getters.getSearchFilterGenerateOrder
        return freightDocumentTypesId
      },
      // setter
      set(newValue) {
        store.commit('updateAttributeCriteriaGenerateOrder', {
          attribute: 'freightDocumentTypesId',
          value: newValue
        })
      }
    })
    ...
    const optionsList = computed({
      get() {
        const { listfreightDocumentTypes } = store.getters.getSearchFilterGenerateOrder
        if (!isEmptyValue(listfreightDocumentTypes)) {
          return listfreightDocumentTypes
        }
        return []
      },
      set(newValue) {
        store.commit('updateAttributeCriteriaGenerateOrder', {
          attribute: 'listfreightDocumentTypes',
          value: newValue
        })
      }
    })
