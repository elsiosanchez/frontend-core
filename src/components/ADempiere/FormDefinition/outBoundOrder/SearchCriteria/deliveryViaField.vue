...
<template>
  ...
  >
    <template slot="label">
      {{ $t('form.outBoundOrder.searchCriteria.deliveryVia') }}
    </template>
    <el-select
      v-model="value"
      clearable
...
export default defineComponent({
  ...
  setup() {
    const value = computed({
      // getter
      get() {
        const { deliveryViaId } = store.getters.getSearchFilterGenerateOrder
        return deliveryViaId
      },
      // setter
      set(newValue) {
        store.commit('updateAttributeCriteriaGenerateOrder', {
          attribute: 'deliveryViaId',
          value: newValue
        })
      }
    })
