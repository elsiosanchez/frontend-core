...
<template>
  ...
  >
    <template slot="label">
      {{ $t('form.outBoundOrder.searchCriteria.salesRepresentative') }}
    </template>
    <el-select
      v-model="value"
      clearable
...
export default defineComponent({
  ...
  setup() {
    const value = computed({
      // getter
      get() {
        const { salesRepresentativeId } = store.getters.getSearchFilterGenerateOrder
    ...
    const optionsList = computed({
      ...
      set(newValue) {
        store.commit('updateAttributeCriteriaGenerateOrder', {
          attribute: 'listSalesRepresentative',
          value: newValue
        })
      }
    })
