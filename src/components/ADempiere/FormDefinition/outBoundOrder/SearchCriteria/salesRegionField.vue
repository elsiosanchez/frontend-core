...
<template>
  ...
  >
    <template slot="label">
      {{ $t('form.outBoundOrder.searchCriteria.salesRegion') }}
    </template>
    <el-select
      v-model="value"
      clearable
...
export default defineComponent({
  ...
  setup() {
    const value = computed({
      // getter
      get() {
        const { salesRegionId } = store.getters.getSearchFilterGenerateOrder
        return salesRegionId
      },
      // setter
      set(newValue) {
        store.commit('updateAttributeCriteriaGenerateOrder', {
          attribute: 'salesRegionId',
          value: newValue
        })
      }
    })
    const optionsList = computed({
      get() {
        const { listSalesRegion } = store.getters.getSearchFilterGenerateOrder
        if (!isEmptyValue(listSalesRegion)) {
          if (listSalesRegion.some(item => item.label === undefined)) {
            const listFormData = listSalesRegion.map(item => {
              return {
                id: item.id,
