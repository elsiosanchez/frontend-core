...
export default defineComponent({
  ...
  setup() {
    ...
    const getItemQuantity = computed(() => {
      const list = currentGiftCard.value.gift_card_lines
      if (list) {
        const result = list.map(list => convertToNumber(list.quantity_entered))
    ...
    const numberOfLines = computed(() => {
      return currentGiftCard.value.gift_card_lines.length
    })
