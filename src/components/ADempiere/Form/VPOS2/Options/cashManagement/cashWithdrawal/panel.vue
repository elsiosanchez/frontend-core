...
import paymentMethods from '@/components/ADempiere/Form/VPOS2/Options/cashManagement/cashWithdrawal/fields/paymentMethods.vue'
import collectionAgent from '@/components/ADempiere/Form/VPOS2/Options/cashManagement/cashWithdrawal/fields/collectionAgent.vue'
import cashBank from '@/components/ADempiere/Form/VPOS2/Options/cashManagement/cashWithdrawal/fields/cashBank.vue'
import currency from '@/components/ADempiere/Form/VPOS2/Options/cashManagement/cashWithdrawal/fields/currency.vue'
...
export default defineComponent({
  ...
  setup() {
    ...
    const currencyPayment = computed(() => {
      const currency = store.getters.getAttributeCashWithdrawalFields({
        attribute: 'currency'
      })
    ...
    function addPayment() {
      ...
      const paymentMethods = store.getters.getAttributeCashWithdrawalFields({
      ...
      store.dispatch('createPaymentWithdrawal', {
        ...
        charge_id: default_withdrawal_charge_id,
    ...
    function deletePaymentsOpen(payment) {
      return store.dispatch('deletePaymentsWithdrawal', { payment_id: payment.id })
        .finally(() => {
          return false
        })
    }
    ...
    function setDefaultPosData() {
      ...
      if (isEmptyValue(currencyPayment.value.iso_code) && !isEmptyValue(price_list)) {
        store.commit('setAttributeCashWithdrawalFields', { attribute: 'currency', value: price_list.currency })
      }
      if (isEmptyValue(collectionAgent.value) && !isEmptyValue(currenUser)) {
        store.commit('setAttributeCashWithdrawalFields', { attribute: 'collectionAgent', value: currenUser })
      }
    }
